...
export default function ProjectionOverlay() {
  useEffect(() => {
    ...
    const mount = () => {
      ...
      one.style.position='relative'; two.style.position='relative';
    ...
    const fetchFrame=()=>fetch(`${API}/api/frame`).then(r=>r.ok?r.json():null).then(d=>{if(!dead){frame=d;render();}}).catch(()=>{});
    ...
    const render=()=>{
      ...
      if(vals.length){
        ...
        if(axis==='x'){r.setAttribute('x',px(lo));r.setAttribute('y',h/2-11);r.setAttribute('width',Math.max(1,px(hi)-px(lo)));r.setAttribute('height',22);} else {r.setAttribute('x',w/2-11);r.setAttribute('y',h/2-hi*h/2);r.setAttribute('width',22);r.setAttribute('height',Math.max(1,(hi-lo)*h/2));}
        r.setAttribute('fill','#788f98');r.setAttribute('fill-opacity','.9');r.setAttribute('stroke','#9bf5ff');r.setAttribute('stroke-width','1.5');r.setAttribute('rx','2');s1.appendChild(r);
      ...
      FACES.forEach((face,fi)=>{const pp=face.map(i=>pts[i]);if(pp.some(p=>!p))return;const poly=document.createElementNS('http://www.w3.org/2000/svg','polygon');poly.setAttribute('points',pp.map(xy).map(a=>a.join(',')).join(' '));poly.setAttribute('fill','#788f98');poly.setAttribute('fill-opacity',String(.18+fi*.015));poly.setAttribute('stroke','#9bf5ff');poly.setAttribute('stroke-width','1');s2.appendChild(poly);});
