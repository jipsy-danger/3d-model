...
function OneDView({ heading }) {
  ...
  return <article className="view-card one-d-card">
    <div className="card-head"><span>1D</span><small>X-axis projection · point + line · 3D linked</small><span className="angle-readout">{String(Math.round(heading)).padStart(3, '0')}°</span></div>
    <div className="one-d-space"><div className="one-d-plot-grid"/><div className="one-d-axis"/><div className="one-d-zero"/>
      {mode === 'points' && points.map((p, i) => <i key={i} className="one-d-cloud-dot" style={{ left: `${p.x}%`, top: `${p.y}%` }}/>) }
      {mode === 'line' && <div className="one-d-line-profile"/>}
      <span className="one-d-label x-label">X (meters)</span><span className="one-d-label y1-label">Intensity / Height</span>
    </div>
    <div className="one-d-controls"><button className={mode === 'points' ? 'selected' : ''} onClick={() => setMode('points')}>✦ POINTS</button><button className={mode === 'line' ? 'selected' : ''} onClick={() => setMode('line')}>⌁ LINE</button><span className="auto-toggle"><b/> AUTO SCALE</span><span className="grow"/><span className="axis-select">X-AXIS</span></div>
  </article>;
}
...
function TwoDView({ heading }) {
  ...
  return <article className="view-card two-d-card">
    <div className="card-head"><span>2D</span><small>X/Y plane · rectangle · square · circle · 360°</small><span className="angle-readout">{String(Math.round(heading)).padStart(3, '0')}°</span></div>
    <div className="two-d-space"><div className="two-d-grid-lines"/><div className="two-d-azimuth-ruler"><div className="two-d-ruler-ring">
      {rulerDegrees.map((degree) => { const a = degToRad(degree); const x = 50 + Math.sin(a) * 50; const y = 50 - Math.cos(a) * 50; return <span key={`tick-${degree}`} className={`two-d-ruler-tick ${degree % 30 === 0 ? 'major' : ''}`} style={{ left: `${x}%`, top: `${y}%`, transform: `translate(-50%, -50%) rotate(${degree}deg)` }}/>; })}
      {majorDegrees.map((degree) => { const a = degToRad(degree); const x = 50 + Math.sin(a) * 56; const y = 50 - Math.cos(a) * 56; return <span key={`label-${degree}`} className="two-d-ruler-label" style={{ left: `${x}%`, top: `${y}%`, transform: 'translate(-50%, -50%)' }}>{degree}°</span>; })}
      <span className="two-d-ruler-label two-d-ruler-360">360°</span>
    </div></div><div className="two-d-center"/>
      {shape === 'points' && Array.from({ length: 60 }, (_, i) => <i key={i} className="two-d-cloud-point" style={{ left: `${50 + Math.sin(i * 0.57) * 34}%`, top: `${50 + Math.cos(i * 0.43) * 25}%` }}/>) }
      {shape === 'rect' && <div className="shape-rect"/>}{shape === 'square' && <div className="shape-square"/>}{shape === 'circle' && <div className="shape-circle"/>}
      <div className="two-d-heading" style={{ transform: `translate(-50%, -100%) rotate(${heading}deg)` }}/><span className="two-d-x-label">X</span><span className="two-d-y-label">Y</span>
    </div>
    <div className="two-d-controls"><button className={shape === 'points' ? 'selected' : ''} onClick={() => setShape('points')}>✦ POINTS</button><button className={shape === 'rect' ? 'selected' : ''} onClick={() => setShape('rect')}>RECT</button><button className={shape === 'square' ? 'selected' : ''} onClick={() => setShape('square')}>SQUARE</button><button className={shape === 'circle' ? 'selected' : ''} onClick={() => setShape('circle')}>CIRCLE</button><span className="grow"/><span className="auto-toggle"><b/> GRID</span></div>
  </article>;
}
...
function TwoFiveDView({ heading }) {
  ...
  return <article className="view-card two-five-card"><div className="card-head"><span>2.5D</span><small>Reserved · backend range grid · 360° ruler</small></div>
    <div className="grid-wrap"><div className="grid-surface" ref={grid}><div className="grid-floor"/>{Array.from({ length: 9 }, (_, i) => <div className="scan-row" key={`r${i}`} style={{ top: `${10 + i * 10}%` }}/>) }{Array.from({ length: 11 }, (_, i) => <div className="scan-col" key={`c${i}`} style={{ left: `${5 + i * 9}%` }}/>) }<div className="fov-ring ring-a"/><div className="fov-ring ring-b"/><div className="fov-ring ring-c"/><div className="object"/><div className="ego"/></div>
      <div className="ruler">{[0,30,60,90,120,150,180,210,240,270,300,330,360].map((d) => <span key={d} style={{ '--pos': `${d / 3.6}` }}>{d}°</span>)}<div className="ruler-pointer" style={{ '--pointer': `${heading / 3.6}%` }}/></div>
    </div><div className="two-five-controls"><span>PAN · ROTATE · MEASURE</span><span className="grow"/><span>2.5D NEXT STAGE</span></div>
  </article>;
}
...
function add3DRuler(scene) {
  ...
  for (let i = 0; i <= 128; i += 1) { const a = i / 128 * Math.PI * 2; pts.push(new THREE.Vector3(-Math.sin(a) * radius, -1.485, Math.cos(a) * radius)); }
  ...
  for (let d = 0; d <= 360; d += 10) {
    ...
    group.add(new THREE.Line(new THREE.BufferGeometry().setFromPoints([new THREE.Vector3(-Math.sin(a) * inner, -1.48, Math.cos(a) * inner), new THREE.Vector3(-Math.sin(a) * outer, -1.48, Math.cos(a) * outer)]), new THREE.LineBasicMaterial({ color: major ? 0x8fe9f5 : 0x456b75, transparent: true, opacity: major ? .9 : .6 })));
    if (major) { const s = makeDegreeSprite(`${d}°`), r = radius + .58; s.position.set(-Math.sin(a) * r, -1.43, Math.cos(a) * r); if (d === 360) { s.position.x -= .3; s.position.z += .3; } group.add(s); }
  }
  scene.add(group);
}
...
function ThreeView({ onHeading, resetRef, rotateRef }) {
  ...
  useEffect(() => {
    ...
    const defaultPosition = new THREE.Vector3(0, 5.5, -7.5); const defaultTarget = new THREE.Vector3(0, -.35, 0); camera.position.copy(defaultPosition);
    ...
    const floor = new THREE.GridHelper(18, 36, 0x31515c, 0x172a31); floor.position.y = -1.55; scene.add(floor); const axes = new THREE.AxesHelper(3.5); axes.position.set(-4, -1.54, -4); scene.add(axes); const base = new THREE.Mesh(new THREE.CircleGeometry(2.8, 64), new THREE.MeshBasicMaterial({ color: 0x0c171b, transparent: true, opacity: .75, side: THREE.DoubleSide })); base.rotation.x = -Math.PI / 2; base.position.y = -1.54; scene.add(base); add3DRuler(scene);

    const solidMaterial = new THREE.MeshStandardMaterial({ color: 0x7896a0, metalness: .12, roughness: .34, transparent: true, opacity: .82, side: THREE.DoubleSide });
    ...
    const renderRaw = (raw) => {
      ...
      solid = new THREE.Mesh(geometry, solidMaterial); solid.position.y = -.4; scene.add(solid);
      wire = new THREE.LineSegments(new THREE.EdgesGeometry(geometry), wireMaterial); wire.position.y = -.4; wire.scale.setScalar(1.002); scene.add(wire);
      const origin = Array.isArray(raw.origin) ? raw.origin.map(Number) : [0,0,0];
      centroid = new THREE.Mesh(new THREE.SphereGeometry(.1, 20, 20), centroidMaterial); centroid.position.set(origin[0] || 0, (origin[1] || 0) - .4, origin[2] || 0); scene.add(centroid);
    };
    const fallbackRaw = { origin:[0,0,0], vertices:[[-1.15,-1.15,-1.15],[1.15,-1.15,-1.15],[1.15,1.15,-1.15],[-1.15,1.15,-1.15],[-1.15,-1.15,1.15],[1.15,-1.15,1.15],[1.15,1.15,1.15],[-1.15,1.15,1.15]] };
    ...
    fetch(`${API}/api/frame`).then(r => r.ok ? r.json() : null).then(d => { if (dead || !d?.input) return; renderRaw(d.input); }).catch(() => {});
...
export default function ViewerV2() {
  ...
  return <main className="app-shell">
    ...
    <section className="analysis panel"><div><span className="section-kicker">FOV / ORIENTATION</span><strong>{Math.round(heading)}°</strong><small>linked heading</small></div><div><span className="section-kicker">INPUT</span><strong>RAW CUBE</strong><small>backend/data/raw/cube.json</small></div><div><span className="section-kicker">RULER</span><strong>CLOCKWISE 0° → 360°</strong><small>0° top · 90° right · 180° bottom · 270° left</small></div></section>
  </main>;
}
