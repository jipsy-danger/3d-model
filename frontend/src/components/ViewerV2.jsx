...
function OneDView({ heading, pitch, cameraRight, cameraUp, cameraForward, cameraPosition, cameraFov, cameraAspect, sync }) {
  const mount = useRef(null);
  const dataRef = useRef(null);
  const viewRef = useRef({ center: 0, scale: 1 });
  ...
  useEffect(() => {
    ...
    if (!root) return;
    const down = (e) => {
      if (sync || e.button !== 0) return;
      dragRef.current = { x: e.clientX, center: viewRef.current.center };
      root.setPointerCapture?.(e.pointerId);
    };
    const move = (e) => {
      if (sync || !dragRef.current) return;
      const width = Math.max(1, root.clientWidth);
      viewRef.current.center = dragRef.current.center - (e.clientX - dragRef.current.x) / width * (1 / viewRef.current.scale);
      redraw((v) => v + 1);
    };
    ...
    const wheel = (e) => {
      if (sync) return;
      e.preventDefault();
      viewRef.current.scale = Math.min(20, Math.max(.15, viewRef.current.scale * Math.exp(-e.deltaY * .0015)));
      redraw((v) => v + 1);
    };
  ...
  const forward = new THREE.Vector3(...(cameraForward || right.clone().cross(up).toArray())).normalize();
  ...
  const project = (point) => {
    ...
    const sx = p.dot(right) / (depth * tanHalf * aspect);
    const sy = p.dot(up) / (depth * tanHalf);
    return axis === 'x' ? sx : sy;
  };

  const projected = points.map(project);
  const visible = projected.filter((v) => v !== null);
  const cameraCenter = sync ? 0 : viewRef.current.center;
  const cameraScale = sync ? 1 : viewRef.current.scale;
  ...
  const min = visible.length ? Math.min(...visible) : -1;
  const max = visible.length ? Math.max(...visible) : 1;
  const span = Math.max(.001, max - min);
  const label = axis.toUpperCase();
  const projectionLabel = axis === 'x'
    ? `SCREEN X · CAMERA RIGHT · AZ ${Math.round(heading)}°`
    : `SCREEN Y · CAMERA UP · AZ ${Math.round(heading)}° · PITCH ${Math.round(pitch || 0)}°`;
  ...
  return <article className="view-card one-d-card">
    <div className="card-head">
      <span>1D</span>
      <small>ONE-DIMENSIONAL SPATIAL PROJECTION · {label}-AXIS</small>
      <span className="angle-readout">{String(Math.round(heading)).padStart(3, '0')}°</span>
    </div>
    <div className="one-d-space one-d-interactive" ref={mount}>
      ...
      {centroidProjected !== null && <i className="one-d-centroid" style={centroidStyle} />}
      <span className="one-d-sync-indicator">● {sync ? '3D CAMERA SYNC' : 'LOCAL 1D VIEW'} · {Math.round(heading)}° · PITCH {Math.round(pitch || 0)}°</span>
      <span className="one-d-space-axis-label">1D SPACE · {label} · {sync ? 'EXACT PERSPECTIVE PROJECTION' : 'PAN + ZOOM'} · {projectionLabel}</span>
      {!points.length && <span className="one-d-viewport-hint">LOADING BACKEND POINTS…</span>}
      <span className="one-d-viewport-hint" style={{ bottom: 27 }}>{span.toFixed(2)} m PROJECTED SPAN</span>
    </div>
...
function add3DRuler(scene, floorY) {
  const radius = 3.28, group = new THREE.Group(), pts = [];
...
function buildRawCube(raw) {
  const vertices = raw?.vertices; if (!Array.isArray(vertices) || vertices.length !== 8) return null;
  const geometry = new THREE.BufferGeometry();
  geometry.setAttribute('position', new THREE.BufferAttribute(new Float32Array(vertices.flat().map(Number)), 3));
  geometry.setIndex([0, 1, 2, 0, 2, 3, 4, 6, 5, 4, 7, 6, 0, 4, 5, 0, 5, 1, 3, 2, 6, 3, 6, 7, 1, 5, 6, 1, 6, 2, 0, 3, 7, 0, 7, 4]); geometry.computeVertexNormals(); return geometry;
}
...
function ThreeView({ onViewChange, resetRef, rotateRef }) {
  ...
  useEffect(() => {
    ...
    const defaultPosition = new THREE.Vector3(0, 5.5, -7.5), defaultTarget = new THREE.Vector3(0, 0, 0); camera.position.copy(defaultPosition);
    ...
    const floorY = -1.15, floor = new THREE.GridHelper(18, 36, 0x31515c, 0x172a31); floor.position.y = floorY; scene.add(floor);
    ...
    const renderRaw = (raw, centroidPoint) => {
      ...
      solid = new THREE.Mesh(geometry, solidMaterial); scene.add(solid); wire = new THREE.LineSegments(new THREE.EdgesGeometry(geometry), wireMaterial); wire.scale.setScalar(1.001); scene.add(wire);
      const p = Array.isArray(centroidPoint) ? centroidPoint.map(Number) : [0, 0, 0]; xAxis = new THREE.Line(new THREE.BufferGeometry().setFromPoints([new THREE.Vector3(-100, p[1] || 0, p[2] || 0), new THREE.Vector3(100, p[1] || 0, p[2] || 0)]), axisMaterial); xAxis.renderOrder = 999; scene.add(xAxis);
      centroid = new THREE.Mesh(new THREE.SphereGeometry(.17, 24, 24), centroidMaterial); centroid.position.set(p[0] || 0, p[1] || 0, p[2] || 0); centroid.renderOrder = 1000; scene.add(centroid);
    };
    ...
    const changed = () => {
      const o = camera.position.clone().sub(controls.target), h = Math.hypot(o.x, o.z), heading = h > .0001 ? norm(Math.atan2(o.x, o.z) * 180 / Math.PI) : 180, pitch = Math.atan2(o.y, h) * 180 / Math.PI;
      const right = new THREE.Vector3(1, 0, 0).applyQuaternion(camera.quaternion).normalize();
      const up = new THREE.Vector3(0, 1, 0).applyQuaternion(camera.quaternion).normalize();
      const forward = new THREE.Vector3(0, 0, -1).applyQuaternion(camera.quaternion).normalize();
      viewCallback.current(heading, pitch, right.toArray(), up.toArray(), forward.toArray(), camera.position.toArray(), controls.target.toArray(), camera.fov, camera.aspect);
    };
    ...
    const rotate3D = (amount) => { const o = camera.position.clone().sub(controls.target), a = degToRad(amount), x = o.x * Math.cos(a) + o.z * Math.sin(a), z = -o.x * Math.sin(a) + o.z * Math.cos(a); camera.position.set(controls.target.x + x, camera.position.y, controls.target.z + z); controls.update(); changed(); };
    if (resetRef) resetRef.current = reset3D; if (rotateRef) rotateRef.current = rotate3D;
    const resize = () => { const w = Math.max(1, root.clientWidth), h = Math.max(1, root.clientHeight); renderer.setSize(w, h, false); camera.aspect = w / h; camera.updateProjectionMatrix(); changed(); };
...
function TwoDView({ heading }) {
  const [shape, setShape] = useState('points'); const rulerDegrees = Array.from({ length: 36 }, (_, i) => i * 10), majorDegrees = Array.from({ length: 12 }, (_, i) => i * 30);
...
export default function ViewerV2() {
  ...
  const [cameraRight, setCameraRight] = useState([1, 0, 0]), [cameraUp, setCameraUp] = useState([0, 1, 0]), [cameraForward, setCameraForward] = useState([0, 0, 1]), [cameraPosition, setCameraPosition] = useState([0, 5.5, -7.5]), [cameraTarget, setCameraTarget] = useState([0, 0, 0]);
  ...
  const publish = (h, p, right, up, forward, position, target, fov, aspect) => {
    if (!sync || socket.current?.readyState !== WebSocket.OPEN) return;
    socket.current.send(JSON.stringify({ type: 'view_state', heading: norm(h), pitch: Number(p), right, up, forward, position, target, fov: Number(fov), aspect: Number(aspect) }));
  };

  useEffect(() => {
    const ws = new WebSocket(API.replace(/^http/, 'ws') + '/ws/view'); socket.current = ws;
    ws.onmessage = e => { try { const m = JSON.parse(e.data); if (!sync) return; if (typeof m.heading === 'number') setHeading(norm(m.heading)); if (typeof m.pitch === 'number') setPitch(m.pitch); if (Array.isArray(m.right)) setCameraRight(m.right); if (Array.isArray(m.up)) setCameraUp(m.up); if (Array.isArray(m.forward)) setCameraForward(m.forward); if (Array.isArray(m.position)) setCameraPosition(m.position); if (Array.isArray(m.target)) setCameraTarget(m.target); if (typeof m.fov === 'number') setCameraFov(m.fov); if (typeof m.aspect === 'number') setCameraAspect(m.aspect); } catch {} };
    return () => { ws.close(); socket.current = null; };
  }, [sync]);

  const handleThreeView = (h, p, right, up, forward, position, target, fov, aspect) => {
    const hv = norm(h), pv = Number(p), rr = right || [1, 0, 0], uu = up || [0, 1, 0], ff = forward || [0, 0, 1], pp = position || [0, 5.5, -7.5], tt = target || [0, 0, 0], fv = Number(fov || 48), ar = Number(aspect || 1);
    setThreeHeading(hv); setThreePitch(pv); setCameraRight(rr); setCameraUp(uu); setCameraForward(ff); setCameraPosition(pp); setCameraTarget(tt); setCameraFov(fv); setCameraAspect(ar);
    if (sync) { setHeading(hv); setPitch(pv); publish(hv, pv, rr, uu, ff, pp, tt, fv, ar); }
  };
  ...
  const toggleSync = () => setSync(enabled => {
    ...
    if (next) {
      ...
      window.setTimeout(() => { if (socket.current?.readyState === WebSocket.OPEN) socket.current.send(JSON.stringify({ type: 'view_state', heading: norm(threeHeading), pitch: Number(threePitch), right: cameraRight, up: cameraUp, forward: cameraForward, position: cameraPosition, target: cameraTarget, fov: cameraFov, aspect: cameraAspect })); }, 0);
    }
    return next;
  });

  const rotateBy = v => threeRotateRef.current ? threeRotateRef.current(v) : handleThreeView(threeHeading + v, threePitch, cameraRight, cameraUp, cameraForward, cameraPosition, cameraTarget, cameraFov, cameraAspect);
  const resetAll = () => threeResetRef.current ? threeResetRef.current() : handleThreeView(180, 28, [1, 0, 0], [0, 1, 0], [0, 0, 1], [0, 5.5, -7.5], [0, 0, 0], 48, cameraAspect);
